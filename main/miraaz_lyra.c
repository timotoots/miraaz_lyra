...
void app_main(void)
{
    ...
    for (int i = 0; i < NUMBER_SOURCE_FILE; i++) {
        ...
        ESP_LOGI(TAG, "[ * ] Read file : %s", str_name);
    ...
    while (1) {
        ...
        for (int i = 0; i < NUMBER_SOURCE_FILE; i++) {
            ...
            if (msg.source_type == AUDIO_ELEMENT_TYPE_ELEMENT && msg.source == (void *)wav_decoder[i]
              ...
              //   && msg.cmd == AEL_IO_DONE
                // ESP_LOGW(TAG, "[ * ] STREAM I: %i", i);
                // ESP_LOGW(TAG, "[ * ] CMD: %i", msg.cmd);
                // ESP_LOGW(TAG, "[ * ] DATA: %i", (int)msg.data);
            ...
            && msg.cmd == AEL_MSG_CMD_REPORT_STATUS && (((int)msg.data == AEL_STATUS_STATE_STOPPED)
                    || ((int)msg.data == AEL_STATUS_STATE_FINISHED))) {
                        ...
                        ESP_LOGI(TAG, "LOOP FILE %i",i);

                        // Reset pipeline for looping
                        audio_pipeline_reset_ringbuffer(pipeline[i]);
                        audio_pipeline_reset_elements(pipeline[i]);
                        audio_pipeline_resume(pipeline[i]);
        
                        // change volume
                       // float gains[2] = {-20,-20};
                      //  downmix_set_gain_info(downmixer,gains,1);
        ...
        // ESP_LOGI(TAG, "main while %s",msg.cmd);
